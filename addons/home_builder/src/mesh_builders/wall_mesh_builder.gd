...
##
## The silhouette is rectangular throughout: _build_face's band scan emits
## axis-aligned quads and _build_edges caps the top with a single one. Gable
## ends — a wall whose top follows a roof slope — are therefore generated by
## RoofMeshBuilder instead, which also keeps them in sync with the roof's
## width and pitch for free. The one case that cannot cover is an opening in
## a gable end; if that ever comes up, the way in is an optional top profile
## here — a list of (x, y) breakpoints for the band scan to clip against —
## rather than a parallel wall type, so openings, miters, materials and
## gizmos keep working.
